...
TaxLens is NOT tax preparation, NOT a general accounting package, NOT a chatbot, NOT an open-ended rules engine. It IS aggressive within US tax law — the agent claims defensible deductions (including strategic Cohan for §162 cases) rather than leaving them on the table. The app prefers the *better-documented* position when there's a real choice; it does not back away from supportable §162 cases just because they could be challenged. Fraud is a hard line — no fabricated attendees, no fabricated business purposes.
...
3. **AI decides by default.** The autonomous CPA agent (`lib/ai/cpaAgent.ts`) classifies every transaction in one Sonnet pass and emits a single audit memo to the client's Documents folder. When §274(d) substantiation is missing (meals/travel/vehicle/gifts/listed property without attendees, purpose, place), the AI defaults the row to PERSONAL with a `notClaimedReason` so the taxpayer can promote it later by uploading a receipt or note. STOPs as a model still exist in the schema as a rare escape hatch (e.g. 1099-K reconciliation upload, period-gap break) — but they are NOT generated for routine missing-substantiation cases. *(Phase 1 rewrite, Apr 2026 — superseded the original "Silence is a bug; STOP is a feature" rule, which over-blocked the user with 60+ STOPs per year.)*
...
7. **Cohan is allowed strategically — never for §274(d).** The CPA agent may use Cohan for §162 expenses where evidence tier 3 is supportable; classifications carry `cohanFlag=true` so the audit memo highlights them. §274(d) categories (meals, travel, vehicle, gifts, listed property) STILL require contemporaneous substantiation — Cohan is never an excuse there, and the app never labels reconstructed §274(d) evidence as contemporaneous. Tier-4 reliance overall is flagged in the risk score and the audit memo. *(Phase 1 rewrite — superseded the original "Cohan is a rescue, not a strategy" rule, which left defensible §162 deductions on the table.)*
...
10. **V1 supports five entity types.** Schedule C (sole prop / single-member LLC), Form 1120-S + Schedule K-1 (S-Corp), Form 1065 + K-1 (multi-member LLC / partnership), and Form 1120 (C-Corp). The form spec lives in `lib/forms/registry.ts` — `getFormSpec(entityType)` returns the line allowlist + K-1 + SE-tax + payroll posture. The CPA agent reads the registry and emits the right form-specific guidance. *(Phase 2-4 rewrite — superseded the original "one entity type, Schedule C only" rule. K-1s currently default to single-owner 100% allocation; per-owner Shareholder/Partner records are a follow-up.)*
...
## CPA-review round-5 notes (2026-05-09 — Atif review II)
...
- **Migration `20260509_add_stop_ai_suggestion`** — `StopItem.aiSuggestion Json?`. Persisted so a low-confidence Sonnet decision still leaves the user a one-click default on the next render (instead of four blank radios).
- **`lib/stops/aiSuggestion.ts` (new)** — single derivation entry point covering MERCHANT (existing logic), TRANSFER (Wise/Pocketsflow/Apple-Cash heuristics), DEPOSIT (Stripe/PayPal → PLATFORM_1099, eBay/marketplace → CLIENT, refund/reversal → REFUND). Persisted `stop.aiSuggestion` overrides every heuristic; malformed JSON sanitizes to `null`. Heuristic order: Pocketsflow + Apple Cash matched **before** the WISE/TRANSFER pattern because the raw strings can contain "TRANSFER" — more-specific signal wins. Also exports `aiSuggestionFromResolution` so `autoResolveStops` can persist its sub-threshold decisions in the same shape.
- **`app/(app)/years/[year]/stops/page.tsx`** — reads `deriveAiSuggestion` for every stop category instead of only MERCHANT.
- **`stops-client.tsx`** — extended `AiSuggestion` union type lives in the new module (re-exported here for back-compat). `TransferForm` + `DepositForm` now show the same "AI suggests …" banner and pre-select the radio. `MerchantForm` uses the unioned suggestion's `kind` discriminator to keep its `scheduleCLine` access type-safe. `useSubmit` calls `router.refresh()` after `resolveStop` / `deferStop` so the ledger / counts update without a hard reload.
- **`stops/actions.ts`** — `AUTO_RESOLVE_CONFIDENCE_THRESHOLD` lowered from 0.85 → 0.70. Below threshold, the action now persists `aiSuggestion` to the row before skipping. After `resolveStop` with `applyToSimilar=true`, the after-hook archives any *other* PENDING StopItem on the same `merchantRuleId` (was leaving 27 duplicates after the user resolved 1 of 28 Wise stops). `autoResolveStops` now calls `deriveStopsFromAssertions` so DEPOSIT-stops materialize for the residual unclassified inflows uncovered by the resolve.
- **`lib/classification/apply.ts`** — `scheduleCLine` fallback when a deductible-coded MerchantRule has `null` line: WRITE_OFF → "Line 27a Other Expenses", TRAVEL → "Line 24a Travel", COGS → "Part III COGS", MEALS → "Line 24b Meals". Without this, `WRITE_OFF` rows like CASH ADVANCE INTEREST CHARGE landed deductible cents on the ledger but didn't roll up to any Schedule C line — A03 / Schedule C totals diverged.
- **`lib/validation/assertions.ts` A04** — now filters `amountNormalized < 0` AND `!isTransferPairedWith` to match A13's gross-receipts logic. Previously A04 used unfiltered `Math.abs()` and disagreed with A13 by hundreds of dollars on Atif's prod ledger ($24,811 vs $18,313). One number, one place.
- **`risk/page.tsx` Critical card contrast** — the `bg-red-50` / `bg-orange-50` light fills were rendering pink-on-pink on dark theme (title text vanished into the muted gray foreground). Switched to translucent `bg-*-500/10` fills, added explicit `severityTitleColor` per severity, and bumped the description text from `text-muted-foreground` to `text-foreground/80`.
- **`pipeline/actions.ts` `runCpaAgentAction`** — calls `deriveStopsFromAssertions` after the agent's commit so DEPOSIT-stops materialize on every fresh agent run (was only running after Apply Rules, which the user rarely clicks separately on the redesigned UI).
...
## Auto-CPA framework notes (2026-05-20)

This phase replaces 9 manual cleanup scripts + an out-of-band Opus 4.7 ledger audit + manual position-memo triggering with an in-app multi-stage AI pipeline. The full plan that drove this work is at `plans/during-the-process-of-radiant-hanrahan.md`.

- **Migration `20260520_add_auto_cpa_framework`** — adds:
  - `LedgerFinding` model (severity/category/proposedAction/state machine: PROPOSED → ACCEPTED → APPLIED, plus DISMISSED, SUPERSEDED). Each finding tracks its source PipelineRun and supersession chain.
  - `Classification.cohanFlag Boolean @default(false)` — explicit column; CLAUDE.md round-5 notes referenced this but it was actually derived from `evidenceTier >= 4`. The migration backfills `cohanFlag=true` on every existing `isCurrent=true` row where `evidenceTier >= 4` and `code IN (WRITE_OFF, WRITE_OFF_TRAVEL, WRITE_OFF_COGS, MEALS_50, MEALS_100, GRAY)`, preserving the audit packet's prior behavior.
  - Six new `PipelineRunKind` values: `PRE_CLEANUP`, `CPA_AUDIT`, `COHAN_SWEEP`, `SUBSTANTIATION_QUEUE`, `FINDINGS_APPLY`, `RELOCK_VERIFY`.
- **`lib/classification/cohanGuards.ts` (new)** — the §274(d) bright-line guard. `assertNot274dCohan` rejects any proposal whose code is MEALS_50/MEALS_100/WRITE_OFF_TRAVEL, whose citations include `§274(d)`, whose `scheduleCLine` starts with Line 24a/24b/9, or whose merchant matches any fragment in `SECTION_274D_MERCHANT_FRAGMENTS` (UBER, LYFT, GAS, FUEL, EXXON, RESTAURANT, STARBUCKS, AIRLINES, HOTEL, MARRIOTT, RENTAL CAR, HERTZ, AIRBNB, DOORDASH, GIFT, FLOWERS, etc.). Called from `cohanSweep.ts` candidate filter, AI-proposal post-filter, `findings/apply.ts` apply gate, AND `cpaAgent.ts` write path — defense in depth at every layer that can set `cohanFlag=true`.
- **`lib/cleanup/preClassification.ts` (new)** — ports 4 .mjs scripts into one Prisma module: `fixInflowMisclassifications` (flip wrongly-coded inflows → NEEDS_CONTEXT), `markOutOfYearStale` (Transaction.isStale on out-of-year rows), `archiveSupersededStops` (PENDING StopItems whose txns now have current Classifications), `backfillDocuments` (Document rows from StatementImports). Idempotent predicate-based. Each sub-step writes its own `PRECLEANUP_*` AuditEvent.
- **`lib/ai/cpaAudit.ts` (new)** — Opus 4.7 ledger audit with Sonnet fallback. Reads a compact ~3 KB summary (per-line totals, top-30 largest, top-30 smallest, PERSONAL > $500, two-sided counterparties) — Opus's job is pattern recognition, not arithmetic. Emits structured `LedgerFinding[]`. Prompt seeded with all 7 of Atif's real findings (bounced check, Wise phantom, missing line, dup buckets, missing W-9, Line 27a DIF risk, PERSONAL outliers). Zod-validated; supersedes prior PROPOSED findings on re-run.
- **`lib/ai/cohanSweep.ts` (new)** — Sonnet 4.6 default; Opus 4.7 when aggregate candidate exposure ≥ $10K. Walks PERSONAL/GRAY/NEEDS_CONTEXT + tier-3 WRITE_OFF rows; loads PriorYearContext for prior-merchant continuity; calls `assertNot274dCohan` BEFORE the AI call AND on every AI-proposed action. Writes `LedgerFinding` with `proposedAction.cohanFlag=true`.
- **`lib/ai/substantiationQueue.ts` (new)** — Sonnet 4.6 generates §274(d) substantiation TEMPLATES. Critical invariant: `aiSuggestion.attendees=""` and `aiSuggestion.purpose=""` are EMPTY by design. The AI proposes the question; the human supplies the facts. Prompt includes good/bad examples to pin the no-fabrication contract.
- **`lib/findings/apply.ts` (new)** — flip-and-insert apply layer for accepted findings. Re-checks `assertNot274dCohan` before any Cohan-flagged write (rejected → `COHAN_FORBIDDEN_REJECTED` AuditEvent). Asserts prior `isCurrent=true` rows still match the finding's view → mismatch becomes SUPERSEDED, not APPLIED. Provides `acceptFinding`, `dismissFinding`, `applyAcceptedFindings`.
- **`lib/lock/relockVerify.ts` (new)** — drift report for re-locks. Compares current per-line Schedule C totals against the prior `TAXYEAR_LOCKED.afterState.perLineTotals`. Thresholds: single line > 15%, gross receipts > 10%, total deductions > 15%. AI narrative cross-check via Sonnet. Throws `DriftApprovalRequiredError` when over threshold; UI catches and shows a confirm dialog.
- **`app/(app)/years/[year]/lock/actions.ts`** — `confirmLock` now (a) runs `RELOCK_VERIFY` when a prior lock exists and bails with `DriftApprovalRequiredError` unless the user has acked via `options.driftAck`, (b) captures `perLineTotals` + `grossReceipts` into the `TAXYEAR_LOCKED.afterState` for the NEXT relock's drift check, (c) auto-generates every position memo (`generateAllPositionMemos`) on lock confirm — previously the user had to navigate to `/memos`.
- **`lib/rules/memoRules.ts`** — new `§162_cohan_sweep` memo type with citations §162, §162(a), §6001, Cohan v. Commissioner 39 F.2d 540, Reg. §1.6001-1, Rev. Proc. 92-71. References §274(d) NEGATIVELY (as the affirmative exclusion). 6 fact checkpoints (aggregate exposure, NAICS nexus, bank visibility, prior-year pattern, §274(d) exclusion, evidence-tier acknowledgment).
- **`lib/ai/positionMemo.ts`** — `gather162CohanFacts` parallels the existing 4 fact gatherers. Iterates current classifications, filters Cohan-flagged + deductible, defense-in-depth-filters §274(d) citations and codes, builds the per-code/per-line breakdown + top-10 claims + affirmative §274(d)-exclusion section. `detectNeededMemos` queues `§162_cohan_sweep` when `cohanFlag=true` count ≥ 10 OR aggregate exposure ≥ $2,500.
- **`/years/[year]/findings`** — review UI for `LedgerFinding` rows. Group by severity; per-finding accept/dismiss; "Accept all auto-fixable" batch; "Apply N accepted findings" runs `applyAcceptedFindings` and reports `applied/superseded/rejected` (rejection means §274(d) hard rail caught a slip-through).
- **`/years/[year]/finalize/auto`** — orchestrator: one click runs PRE_CLEANUP → CPA_AUDIT → COHAN_SWEEP → SUBSTANTIATION_QUEUE sequentially (~60–120s for Atif scale). Drops user on `/findings` to review.
- **Pipeline registrations** — `app/(app)/years/[year]/pipeline/actions.ts` adds `runPreCleanupAction`, `runCpaAuditAction`, `runCohanSweepAction`, `runSubstantiationQueueAction`, `runFindingsApplyAction` wrappers. Each goes through the existing `enqueue()` machinery so stuck-run detection and progress UI keep working.
- **Atif's Cohan-sweep prod execution** — `scripts/atif-cohan-sweep.mjs` (env-gated by `RUN_ATIF_COHAN_SWEEP=true`). Refuses to run if the year is still LOCKED — operator must unlock from the UI first. With `ATIF_COHAN_DRY_RUN=true` (default), runs the 4 pipeline stages and prints findings; with `=false`, also auto-accepts all auto-fixable findings + applies them via flip-and-insert. Re-lock is still manual (operator clicks Confirm Lock from `/lock` — drift dialog surfaces if drift > thresholds; position memos including new §162 Cohan defense memo auto-generate). Wired into `pnpm post-deploy` chain.
- **Tests**: 423 passing (317 prior + 106 new across 8 test files: `cohan-guards`, `cohan-sweep`, `substantiation-queue`, `findings-apply`, `relock-verify`, `position-memo-cohan`, `pre-cleanup`, `cpa-audit`). Pure unit tests for the deterministic logic; DB-integration paths exercised by the prod execution script.
- **§274(d) hard-rail layering** (defense in depth):
  1. `cohanSweep` candidate filter — drops §274(d) merchants/codes BEFORE the AI call.
  2. `cohanSweep` post-AI filter — re-checks the AI's proposed code/line.
  3. `findings/apply` apply-time guard — re-checks once more before any `cohanFlag=true` write.
  4. `cpaAgent.ts` write path — re-checks the agent's own `cohanFlag=true` outputs.
  All four call the same `assertNot274dCohan` function. The 60+ `cohan-guards.test.ts` cases pin every forbidden merchant fragment + every forbidden code + every forbidden Schedule C line.
- **No-§274(d)-fabrication invariant** — `SUBSTANTIATION_QUEUE` queues §274(d) candidates with empty `attendees`/`purpose` fields in the `aiSuggestion`. The user fills them in or the row stays PERSONAL. The AI never generates the facts. Pinned by `substantiation-queue.test.ts`.
- **Verify**: `pnpm test` (423 passing); `pnpm build` clean — 2 new routes (`/years/[year]/findings`, `/years/[year]/finalize/auto`); execute on Atif via `RUN_ATIF_COHAN_SWEEP=true ATIF_COHAN_DRY_RUN=true` on Railway, review `/years/2025/findings`, accept, apply, re-lock from `/years/2025/lock`, observe `§162_cohan_sweep` memo in `Documents` + `03_cohan_labels.csv` populated in the audit packet.
